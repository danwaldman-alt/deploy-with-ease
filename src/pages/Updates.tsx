      ...
      <section className="max-w-4xl mx-auto px-6 py-16 relative z-10">
        ...
        <div className="mb-20">
          {grouped.map(([month, entries], gi) => (
            <motion.div key={month} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: gi * 0.07 }} className="mb-12">
              ...
              <div className="space-y-4 relative">
                <div className="absolute left-[10px] top-2 bottom-2 w-px bg-glass-border/40" />
                {entries.map((entry, i) => {
                  const cfg = typeConfig[entry.type];
                  const TypeIcon = cfg.icon;
                  return (
                    <motion.div key={i} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: gi * 0.07 + i * 0.05 }} className="pl-8 relative">
                      <div className={`absolute left-0 top-4 w-[21px] h-[21px] rounded-full border-2 bg-background flex items-center justify-center ${cfg.ringColor}`}>
                        <div className={`w-2 h-2 rounded-full ${cfg.dotColor}`} />
                      </div>
                      <div className="glass-card p-5 hover:border-primary/15 transition-all duration-300">
                        <div className="flex items-start justify-between gap-4 mb-3 flex-wrap">
                          <div className="flex items-center gap-2.5 flex-wrap">
                            <span className="font-display text-base font-bold text-foreground">{entry.tool}</span>
                            {entry.version && (
                              <span className="text-[10px] font-mono text-muted-foreground/60 px-1.5 py-0.5 rounded bg-secondary/60 border border-glass-border/50">v{entry.version}</span>
                            )}
